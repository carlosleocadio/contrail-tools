...
import openstack
import sys
from networkx import DiGraph, is_strongly_connected, weakly_connected_components
...
#define isomorphic check
def is_isomorphic(controller,vRouter):
    for k in controller:
        if k not in vRouter or set(controller[k])!=set(vRouter[k]):
            return False
    return True
...
def open_ssh_channel(host, user):
    ...
    try:
        ...
        log.info("host : {} is unreachable".format(host))
        return None
    return client
...
def main():
    ...
    log.info("Starting Time {}" .format(current_time))
    # List of Contrail Controller addresses
    ...
    for network_obj in network_objs:
        ...
        if network_obj and network_obj.project_id:
        ...
        if project_obj and project_obj.domain_id:
        ...
        try:
            ...
            log.error("Unable to construct VRF name - Skipping")
            continue
        ...
        print('connections graph is')
        print(len(C))
        print('vrouter graph is')
        print(len(V))
    ...
    now = datetime.now()
    current_time = now.strftime("%Y/%m/%d %H:%M:%S")
    log.info("Ending Time {}" .format(current_time))
